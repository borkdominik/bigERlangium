...
export class NotationEdgeView extends PolylineEdgeView {
    ...
    private createUmlEdge(point:Point, next:Point, relationshipType:string, secondElem: Point, penultimateElem: Point, isSource: boolean):VNode[] {
        const color = isAggregation(relationshipType) ? "var(--vscode-editor-background)" : "var(--vscode-editorActiveLineNumber-foreground)";
        // source and target are required for the rotation
        let source = point;
        let target = secondElem;
        if (relationshipType === RelationshipType.AGGREGATION_RIGHT || relationshipType === RelationshipType.COMPOSITION_RIGHT) {
            source = next;
